...
printf '1.0.0\n' >"$T/version"; printf '# demo\n' >"$T/src/README.md"; printf '{}\n' >"$T/src/bun.lock"; printf '{"name":"fixture"}\n' >"$T/src/package.json"
...
if [ "$1 $2" = 'install --no-save' ] && [ "${FAKE_MUTATE_LOCK:-0}" = 1 ]; then printf 'mutation\n' >> bun.lock; fi
...
config={
 ...
 "HAPI_HOME":f"{t}/home/.hapi", "USE_SUDO":"0", "UPDATE_MODE":"source", "BUN_INSTALL_MODE":"no-save", "SOURCE_REPO":f"{t}/src",
...
grep -q -- 'install --no-save' "$T/bun.log"
...
# Even explicit no-save mode must fail before mutation if Bun changes the lock.
printf '1.0.0\n' >"$T/version"
set +e
FAKE_MUTATE_LOCK=1 PATH="$T/fakebin:$PATH" HAPI_UPDATER_ROOT="$T/root" HAPI_UPDATER_CONFIG="$T/config.json" "$REPO/bin/hapi-safe-update" --force
rc=$?
set -e
test "$rc" != 0
